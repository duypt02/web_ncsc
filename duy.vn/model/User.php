...
class User{
    ...
    public function __construct($name = '', $username ='', $password='', $token='', $id_user=-1, $role=0, $status = 'enable')
    {
        $this->set_id_user($this->test_input($id_user));
        $this->set_name($this->test_input($name));
        $this->set_username($this->test_input($username));
        $this->set_password($this->test_input($password));
    ...
    public function get_users($option ='default', $data = '', $page = 1){
        ...
        if ($option == 'single') {
            $sql = "select * from users where username='{$this->get_username()}'";
        } else if ($option == 'search') {
            $sql = "select * from users 
                where username like '%$data%'";
        } else {
            $sql_get_num_of_pages = "select count(*) from users";
            $sql = "select * from users 
                    limit $so_users_tren_1_trang
                    offset $skip";
        }
        ...
        if (mysqli_num_rows($result) > 0) {
            foreach ($result as $each) {
                $object = new self($each['name'], $each['username'], $each['password'], $each['token'], $each['id_user'], $each['role'], $each['status']);
                $arr[] = $object;
            }
        }
        return $arr;
    ...
    public function insert_user(){
        if($this->checkExistUsername()){
            $sql = "insert into users(username, name, password) values('{$this->get_username()}','{$this->get_name()}', '{$this->get_password()}')";
    ...
    public function update_user(){
        ...
        $sql = "update users 
        set name = '{$this->get_name()}', username = '{$this->get_username()}', password='{$this->get_password()}',role = '{$this->get_role()}', status =  '{$this->get_status()}'
    ...
    public function remove_user(){
        $sql = "delete from users where id_user='{$this->get_id_user()}'";
        (new Connect())->excute($sql);
    }
    ...
    public function checkExistUser($remember){
        $sql = "select * from users where username = '{$this->get_username()}' and password = '{$this->get_password()}'";
        $result = (new Connect())->select($sql);
        $number_rows = mysqli_num_rows($result);
        if($number_rows == 1){
            $each = mysqli_fetch_array($result);
            ...
            if($remember){
                ...
                $sql ="update users set token = '{$this->get_token()}'
                        where
                        username = '{$this->get_username()}'";
                (new Connect())->excute($sql);
    ...
    public function delete_token(){
        ...
        $sql = "update users set token = '' where id_user = '$id_usr'";
        (new Connect())->excute($sql);
    }
